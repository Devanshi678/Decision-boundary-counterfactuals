...
from solver_adam import penalty_method_adam_solver, plot_optimization_history
...
if __name__ == "__main__":
    ...
    # Run Penalty Method with Adam Optimizer IN FEATURE SPACE
    print("Starting Penalty Method with Adam Optimizer IN FEATURE SPACE...\n")
    
    optimized_features, info = penalty_method_adam_solver(
        objective_func=objective,
        constraint_func=constraint,
        x0=x,
        rho_init=10.0,           # Initial penalty parameter
        rho_max=1e8,            # Maximum penalty parameter
        rho_increase=3.0,       # Factor to increase rho
        adam_lr=0.2,           # Adam learning rate (can tune this)
        adam_beta1=0.9,         # Adam beta1 (first moment decay)
        adam_beta2=0.999,       # Adam beta2 (second moment decay)
        inner_max_iter=200,     # Max iterations per inner loop
        outer_max_iter=200,     # Max outer iterations
        tol_constraint=1e-3,    # Tolerance for |F1| < 0.001
        tol_obj=1e-6,           # Tolerance for objective change
        log_interval=20,        # Print every 20 iterations (inner loop)
        verbose=True
    )
    ...
    plot_optimization_history(info, save_path='optimization_history_adam.png')
    ...
    output_image = invert_features_to_image(
        ...
        inv_tv_weight=1e-3,
        inv_cls_weight=1.0,
        save_path="counterfactual_image_adam.jpg"
    )
    ...
    print("  1. counterfactual_image_adam.jpg - Final counterfactual image (Adam)")
    print("  2. optimization_history_adam.png - F1 and F2 convergence plot (Adam)")
    print("="*80)